...
function deterministicPromptVariant(postId: string): "A" | "B" {
  const hash = crypto.createHash("md5").update(postId).digest("hex");
  return parseInt(hash.slice(0, 2), 16) % 2 === 0 ? "A" : "B";
}

function getPromptVariant(post: PostModel): "A" | "B" {
  const title = post.raw.title.toLowerCase();
  if (/\[a\]/.test(title)) return "A";
  if (/\[b\]/.test(title)) return "B";
  return deterministicPromptVariant(post.raw.publicId);
}
...
async function generateAltText(
  ...
  try {
    const promptVariant = getPromptVariant(post);
    const descriptions = await Promise.all(
      post.raw.images.map(async (image, index) => {
        ...
        const ogAltText = result.altText;
        const lines = ogAltText.split("\n").map((line) => `  ${line}`);
        return lines.join("\n\n");
      }),
    );
    ...
    const replyBody = {
      ...
      _sep1: "-----",
      footer:
        `using prompt variant ${promptVariant}. i am a bot, and this action was
        performed automatically. image description were generated by a large
        language model. want to opt out? see [here](${consentUrl}).`.replace(
          /\s+/g,
          " ",
        ),
    };
...
async function main() {
  try {
    ...
    const handleNewImagePost = async (_post: Post) => {
      ...
      if (post.raw.title.includes("[atb]")) {
